...
describe('Debug Box Positions', () => {
  it('dump positions with prioritizeFullLayers=true', () => {
    ...
    result.pallets.forEach((pallet, pIdx) => {
      ...
      sorted.forEach((pb, idx) => {
        ...
        const label = `#${idx + 1}`
        const highlight = idx + 1 >= 58 && idx + 1 <= 63 ? ' <<<<<' : ''
        console.log(
      ...
      for (const [z, lyBoxes] of Array.from(layerMap.entries()).sort(([a], [b]) => a - b)) {
        const types = new Map<string, number>()
        for (const b of lyBoxes) {
        ...
        console.log(`  z=${String(z).padStart(5)} | ${lyBoxes.length} 箱 | ${typeStr}`)
      }
    })
  })
})
