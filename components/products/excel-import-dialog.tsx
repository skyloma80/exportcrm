...
export function ExcelImportDialog({ open, onOpenChange, onSuccess }: ExcelImportDialogProps) {
  ...
  const handleDownloadTemplate = () => {
    ...
    import("xlsx").then((XLSX) => {
      ...
      worksheet["!cols"] = [{ wch: 15 }, { wch: 15 }, { wch: 30 }, { wch: 30 }, { wch: 20 }, { wch: 10 }, { wch: 15 }, { wch: 40 }, { wch: 40 }]
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            <Label>{t("products.import.selectFile")}</Label>
            <div className="border-2 border-dashed rounded-lg p-6 text-center cursor-pointer hover:border-primary/50 transition-colors" onClick={() => fileInputRef.current?.click()}>
              ...
              {file ? (
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">{t("products.import.dropzone")}</p>
                </div>
              )}
            </div>
          </div>
          <div className="flex items-center space-x-2">
            ...
            <Label htmlFor="updateExisting" className="text-sm">{t("products.import.updateExisting")}</Label>
          </div>
          ...
          {result && (
            <Alert variant={result.failed > 0 ? "destructive" : "default"}>
              ...
              <AlertDescription>
                <p>{t("products.import.result", { total: String(result.total), success: String(result.success), failed: String(result.failed) })}</p>
                {result.created > 0 && <p className="text-sm">{t("products.import.created", { count: String(result.created) })}</p>}
                {result.updated > 0 && <p className="text-sm">{t("products.import.updated", { count: String(result.updated) })}</p>}
              </AlertDescription>
            </Alert>
          )}
        </div>
        <DialogFooter>
          ...
          <Button onClick={handleImport} disabled={!file || importing}>
            {importing ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />{t("products.import.importing")}</> : <><Upload className="h-4 w-4 mr-2" />{t("products.import.import")}</>}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
