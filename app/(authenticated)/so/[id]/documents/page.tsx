...
export default function OrderDocumentsPage({ params }: PageProps) {
  ...
  useEffect(() => {
    if (order) {
      setBreadcrumbItems([
        ...
        { label: order.code || t("orders.detail"), href: `/so/${id}${projectIdFromUrl ? `?project=${projectIdFromUrl}` : ''}` },
  ...
  const loadOrderDocuments = async (info: OrderPathInfo) => {
    ...
    try {
      const response = await fetch(`/api/so/${id}/documents?type=order`)
      if (!response.ok) {
  ...
  const loadShipmentDocuments = async (shipment: Shipment, shipmentIndex: number, info: OrderPathInfo) => {
    ...
    try {
      const response = await fetch(`/api/so/${id}/documents?type=shipment&shipmentId=${shipment.id}`)
    ...
    return (
      <div className="p-6">
        <div className="text-center py-12">
          ...
          <Button variant="outline" onClick={() => router.push(`/so/${id}`)} className="mt-4">
            {t("common.back")}
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="p-6 space-y-8">
      ...
      <div className="flex items-center gap-4">
         <Button variant="ghost" size="icon" onClick={() => router.push(`/so/${id}${projectIdFromUrl ? `?project=${projectIdFromUrl}` : ''}`)}>
